...
<!-- SECTION -->
		<div class="section">
			...
			<div class="container">
				  ...
				  <tbody>
				  	...
				  	@foreach($products as $pds)
					    ...
					    <th scope="row">#{{$i}}</th>
					      ...
					      <td>{{number_format($pds->price,0,',','.')}}Đ</td>
					      <td>{{number_format($pds->qty * $pds->price,0,',','.')}}</td>
					      <td>		    
				...
				<h4 class="pull-right">TỔNG TIỀN THANH TOÁN : {{Cart::subtotal()}}<a href="" class="btn-success btn" class="pull-right">THANH TOÁN</a></h4>
			</div>
			<!-- /container -->
		</div>
		<!-- /SECTION -->
