				...
				<div class="row">
					<div class="col-md-12">
						<ul class="breadcrumb-tree">
							<li><a href="trangchu">Trang Chủ</a></li>
							<li class="active"><a href="Phụ Kiện">Phụ Kiện ({{ count($sanphampk) }} Sản Phẩm)</a></li>
						</ul>
					</div>
				</div>
				<!-- /row -->
				...
				<div class="row">
					...
					<div id="store" class="col-md-9">
							...
							<div class="store-filter" style="text-align: left;">
								...
								<div class="store-sort col-lg-3">
										<a>ROM</a>(Red Only Memory)			
											<select name="rom" class="input-select2">
												...
												@foreach($rom as $r)
													@if($r->rom != null)
														<option class="{{Request::get('rom') == $r->rom ? 'selected' : ''}}" {{request()->fullUrlWithQuery(['rom']) == $r->rom ? "selected = 'selected'" : ""}} value="{{ $r->rom }}">{{ $r->rom }}GB
														</option>
													@endif
												@endforeach
											</select>		
								...
								<div class="store-sort col-lg-3">
										<a href="phukien" class="btn btn-primary">Reset bộ lọc</a>
								</div>
								</form>
